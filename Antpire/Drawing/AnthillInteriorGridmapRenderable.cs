    ...
    public void Render(DrawBatch drawBatch, Transform2 trans, Rectangle viewRegion) {
        for (int y = 0; y < GridMap.Grid.GetLength(1); y++) {
            for (int x = 0; x < GridMap.Grid.GetLength(0); x++) {
                GridMap.TilesRenderables[GridMap.Grid[y,x]].Render(drawBatch, new Transform2((new Vector2(x * GridMap.TileWidth + trans.Position.X, y * GridMap.TileWidth + trans.Position.Y))), viewRegion);
            }
        }
    }
}